...
const db=new Database(path.join(DATA,'noviq.sqlite'));
...
function dbRun(sql,args=[]){try{const info=db.prepare(sql).run(...args);return Promise.resolve({id:info.lastInsertRowid,changes:info.changes})}catch(e){return Promise.reject(e)}}
function dbGet(sql,args=[]){try{return Promise.resolve(db.prepare(sql).get(...args))}catch(e){return Promise.reject(e)}}
function dbAll(sql,args=[]){try{return Promise.resolve(db.prepare(sql).all(...args))}catch(e){return Promise.reject(e)}}
...
app.post('/api/control',async(req,res)=>{try{const a=req.body.action;if(a==='stop')stopped=true;if(a==='resume')stopped=false;if(a==='cycle')await Promise.all([strategyCycle(),managerCycle(),workerCycle(),outreachCycle(),followupCycle(),salesReactionCycle(),fulfillmentCycle(),evaluateAgents(),snapshotMetrics()]);if(a==='scanIntelligence')await worldIntelligenceScan();res.json({ok:true,stopped})}catch(e){console.error('control action failed:',e);res.status(500).json({error:String(e.message||e)})}});
